...
export const useOrderDeliveryInfos = (
  getServerTime: () => Date,
  ...
  const [orderDispatchingText, setOrderDispatchingText] =
    React.useState<string>();
  const [isCurrierArrived, setIsCurrierArrived] = React.useState<boolean>(
    order?.dispatchingState === 'arrived-pickup'
  );
  const [isDelivered, setIsDelivered] = React.useState<boolean>(
  ...
  React.useEffect(() => {
    ...
    const getOrderDispatchingKanbanItemText = (
      ...
      if (status === 'confirmed') {
        ...
        if (state === 'arrived-destination')
          result = 'Entreg. no local de entrega';
    ...
    const getOrderDispatchingText = (
      ...
      if (status === 'confirmed') {
        ...
        if (state === 'going-destination')
          result = 'Entregador a caminho da entrega';
        if (state === 'arrived-destination')
          result = 'Entregador no local de entrega';
      } else if (status === 'no-match') result = 'Entregador não encontrado';
      else if (status === 'outsourced') result = 'Logística fora da rede';
      setOrderDispatchingText(result);
    };
    getOrderDispatchingKanbanItemText(
