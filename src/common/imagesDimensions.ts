// business profile
export const logoRatios = [1 / 1];
export const logoResizedWidth = [240];
export const coverRatios = [14 / 5, 3.63];
export const coverResizedWidth = [1008, 1440];
// product datails
export const productRatios = [1 / 1, 7 / 5];
export const productResizedWidth = [288, 1008];
// complements form
export const complementsRatios = [1 / 1];
export const complementsResizedWidth = [288];
// courier (BO)
export const courierSelfieRatios = [1 / 1];
export const courierSelfieResizedWidth = [600];
export const courierDocRatios = [8.5 / 12];
export const courierDocResizedWidth = [600];
// banners
export const bannerWebRatios = [980 / 180];
export const bannerWebResizedWidth = [980];
export const bannerMobileRatios = [320 / 100];
export const bannerMobileResizedWidth = [320];
export const bannerHeroRatios = [1 / 1];
export const bannerHeroResizedWidth = [980];
